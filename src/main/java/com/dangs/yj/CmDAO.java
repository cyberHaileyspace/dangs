...
//			cm_posts = new ArrayList<CmDTO>();
	...
	public static void paging(int pageNum, HttpServletRequest request) {
		...
		request.setAttribute("ment", "게시글이 존재하지 않습니다 ^^..");
...
//				response.getWriter().print("<body>");
	...
	public static void addCm(HttpServletRequest request) throws UnsupportedEncodingException {
		...
		try {
			...
			String sql = "INSERT INTO cm_post (cm_user_id, cm_title, cm_text, cm_cate, cm_size, cm_img) "
					+ "VALUES (?, ?, ?, ?, ?, ?)";
...
//			response.getWriter().print("<body>");
	...
	public static void addReply(HttpServletRequest request, HttpServletResponse response)
		...
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = "INSERT INTO cm_reply (cmr_user_id, cmr_post_no, cmr_text)" + "VALUES (?, ?, ?)";
	...
	public static void GetReply(HttpServletRequest request) {
		String no = request.getParameter("no");
		String sql = "select cr.*, u.USER_PHOTO " + "from cm_reply cr, userdb u, cm_post cp "
				+ "where cr.CMR_POST_NO = cp.CM_NO and " + "      cr.CMR_USER_ID = u.USER_ID and "
				+ "    cmr_post_no = ? order by cr.cmr_date desc";
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			while (rs.next()) {
				...
				reply.setUser_profile(rs.getString(6));
				replies.add(reply);
			}
	...
	public static void radioCm(HttpServletRequest request, HttpServletResponse response) {
		...
		if (val.equals("cm-like")) {
			sql = "select * from cm_post order by cm_like desc";

		} else if (val.equals("cm-view")) {
			sql = "select * from cm_post order by cm_view desc";

		} else if (val.equals("cm-new")) {
			sql = "select * from cm_post order by cm_date";
