...
toggleButtons.forEach((button, index) => {
    button.addEventListener("click", () => {
        ...
        if (moreInfoDiv.classList.contains("active")) {
            ...
            setTimeout(() => (moreInfoDiv.style.display = "none"), 500); // 전환 후 display를 none으로
        } else {
            moreInfoDiv.style.display = "block"; // 먼저 block으로 표시
...
// 요청상세 버튼
document.querySelectorAll(".toggleDetailBtn").forEach((button) => {
  button.addEventListener("click", () => {
    const perOrderDiv = button.closest(".per-order"); // 버튼이 속한 .per-order div 찾기
    const detailsDiv = perOrderDiv.nextElementSibling.nextElementSibling; // 요청상세는 두 번째 sibling

    // active 클래스 토글
    if (detailsDiv.classList.contains("active")) {
      detailsDiv.classList.remove("active"); // 사라지기
      setTimeout(() => (detailsDiv.style.display = "none"), 300); // 전환 후 display를 none으로
    } else {
      detailsDiv.style.display = "block"; // 먼저 flex로 표시
      setTimeout(() => detailsDiv.classList.add("active"), 10); // 부드럽게 보이기
    }
  });
});
